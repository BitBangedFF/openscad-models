/**
 * @file side_support_dimensions.scad
 * @brief TODO.
 *
 */
...
PART_WIDTH = SIDE_SUPPORT_BOARD_WIDTH;
PART_THICKNESS = SIDE_SUPPORT_BOARD_THICKNESS;
    ...
    union()
    {
        ...
        rotate([0, 0, 90])
            union()
            {
                ...
                translate([PART_WIDTH + DIM_SPACE, -dist_a, DIM_HEIGHT])
                    line(
                            length = (2 * DIM_SPACE),
                            width = DIM_LINE_WIDTH,
                            height = 0.01,
        ...
        translate([PART_LENGTH - dist_b, PART_WIDTH + (2 * DIM_SPACE), 0])
            dimensions(dist_b, DIM_LINE_WIDTH, DIM_LEFT);

        rotate([0, 0, 90])
            union()
            {
                translate([PART_WIDTH + DIM_SPACE, -PART_LENGTH + dist_b, DIM_HEIGHT])
                    line(
                            length = (2 * DIM_SPACE),
                            width = DIM_LINE_WIDTH,
                            height = 0.01,
        ...
        translate([dist_a + END_SUPPORT_BOARD_THICKNESS, PART_WIDTH + (5 * DIM_SPACE), 0])
            dimensions(inner_dist, DIM_LINE_WIDTH, DIM_CENTER);

        rotate([0, 0, 90])
            union()
            {
                translate([PART_WIDTH + (4 * DIM_SPACE), -dist_a - END_SUPPORT_BOARD_THICKNESS, DIM_HEIGHT])
                    line(
                            length = (2 * DIM_SPACE),
                            width = DIM_LINE_WIDTH,
                            height = 0.01,
                ...
                translate([PART_WIDTH + (4 * DIM_SPACE), -dist_a - END_SUPPORT_BOARD_THICKNESS - inner_dist, DIM_HEIGHT])
                    line(
                            length = (2 * DIM_SPACE),
                            width = DIM_LINE_WIDTH,
                            height = 0.01,
        ...
        union()
        {
            ...
            translate([PART_LENGTH + DIM_SPACE, PART_WIDTH - dist_c, 0])
                    line(
                            length = (2 * DIM_SPACE),
                            width = DIM_LINE_WIDTH,
                            height = 0.01,
